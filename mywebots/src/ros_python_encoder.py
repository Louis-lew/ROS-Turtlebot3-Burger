...
# Read Encoder
def readEnc(step_time, init_state):
    global encLeft, encRight
    diff = [0.0, 0.0]

    if init_state: #not change in encoder
    ...
    if diff[0] == 0 or diff[1] == 0:
        ...
        left_enc_lin_vel =  WHEEL_RADIUS*diff[0]/step_time
        right_enc_lin_vel = WHEEL_RADIUS*diff[1]/step_time
        enc_lin_vel = -(left_enc_lin_vel+right_enc_lin_vel)/2
        enc_ang_vel = (left_enc_lin_vel-right_enc_lin_vel)/WHEEL_SEPARATION

    encLeft[1] = encLeft[0]
    encRight[1] = encRight[0]
...
def handle_turtle_pose():
    ...
    handle_turtle_static_pose((0., 0., 0.), (0, 0 ,0 ,1), rospy.Time.now(), FRAME_ID, "base_footprint")
    ...
    handle_turtle_static_pose((0., -0.08, 0.033), quaternion_from_euler(-1.57, 0, 0), rospy.Time.now(), "wheel_right_link", FRAME_ID)
